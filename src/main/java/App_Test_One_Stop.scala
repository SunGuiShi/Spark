...
object App_Test_One_Stop {
  ...
  def main(args: Array[String]): Unit = {
    ...
    sql(sqlText = s"select serv_number,cur_income_month,statis_month,brand, " +
      s"is_income,product_type,pack_outer_call_time,pack_inner_call_time, " +
      s"user_id,channel_type,cur_product " +
      ...
      s"and length(serv_number) = 11")
      .join(sql(sqlText = "select distinct serv_number,sex,age,province_code from ods_user_info_m " +
        s"where statis_month = $this_month and length(serv_number) = 11"), usingColumn = "serv_number")
      .join(sql(sqlText = "select distinct serv_number,visit_cnt,prod_id,prod_name," +
        s"total_flow from ods_app_score_m where statis_month = $this_month and length(serv_number) = 11"),
        usingColumn = "serv_number")
      .createTempView(viewName = "tmp_view")

    val app_df: DataFrame = sql(sqlText = "select serv_number,cur_income_month,statis_month,sex,brand,is_income,product_type, " +
      "user_id,channel_type,cur_product,age,province_code,total_flow,visit_cnt,prod_id,prod_name,pack_outer_call_time,pack_inner_call_time "+
      ",case when substring(user_id,1,1) = 'A' then 'cbss' " +
      "when substring(user_id,1,1) = 'B' then 'bss' " +
      "else 'except' end as category from tmp_view where product_type <> '1'")
...
//    app_df
//      .filter(s"cur_product in ('${arr(0)}','${arr(1)}','${arr(2)}','${arr(3)}','${arr(4)}')")
//      .write.mode(SaveMode.Overwrite).saveAsTable(tableName = s"ads_xc_basic")
    ...
    /**
      ...
      */
    sql(sqlText = s"select category,count(distinct serv_number) m0 from " +
      s"ads_app_non_2i_${this_month}_m where age <= 14 and sex = 01  group by category")
      .createTempView(viewName = "t_m0")
    sql(sqlText = s"select category,count(distinct serv_number) m1 from " +
      s"ads_app_non_2i_${this_month}_m where age > 14 and age <= 18 and sex = 01  group by category")
      .createTempView(viewName = "t_m1")
    sql(sqlText = s"select category,count(distinct serv_number) m2 from " +
      s"ads_app_non_2i_${this_month}_m where age > 18 and age <= 35 and sex = 01  group by category")
      .createTempView(viewName = "t_m2")
    sql(sqlText = s"select category,count(distinct serv_number) m3 from " +
      s"ads_app_non_2i_${this_month}_m where age > 35 and age <= 55 and sex = 01  group by category")
      .createTempView(viewName = "t_m3")
    sql(sqlText = s"select category,count(distinct serv_number) m4 from " +
      s"ads_app_non_2i_${this_month}_m where age > 55 and age <= 70 and sex = 01  group by category")
      .createTempView(viewName = "t_m4")
    sql(sqlText = s"select category,count(distinct serv_number) m5 from " +
      s"ads_app_non_2i_${this_month}_m where age > 70 and sex = 01  group by category")
      .createTempView(viewName = "t_m5")


    sql(sqlText = s"select category,count(distinct serv_number) f0 from " +
      s"ads_app_non_2i_${this_month}_m where age <= 14 and sex = 02  group by category")
      .createTempView(viewName = "t_f0")
    sql(sqlText = s"select category,count(distinct serv_number) f1 from " +
      s"ads_app_non_2i_${this_month}_m where age > 14 and age <= 18 and sex = 02  group by category")
      .createTempView(viewName = "t_f1")
    sql(sqlText = s"select category,count(distinct serv_number) f2 from " +
      s"ads_app_non_2i_${this_month}_m where age > 18 and age <= 35 and sex = 02  group by category")
      .createTempView(viewName = "t_f2")
    sql(sqlText = s"select category,count(distinct serv_number) f3 from " +
      s"ads_app_non_2i_${this_month}_m where age > 35 and age <= 55 and sex = 02  group by category")
      .createTempView(viewName = "t_f3")
    sql(sqlText = s"select category,count(distinct serv_number) f4 from " +
      s"ads_app_non_2i_${this_month}_m where age > 55 and age <= 70 and sex = 02  group by category")
      .createTempView(viewName = "t_f4")
    sql(sqlText = s"select category,count(distinct serv_number) f5 from " +
      s"ads_app_non_2i_${this_month}_m where age > 70 and sex = 02  group by category")
    ...
    for (i <- pvc_code.indices) {
      sql(sqlText = s"select category,statis_month," +
        s"count(distinct serv_number) ct_${pvc_code(i)} from " +
        s"ads_app_non_2i_${this_month}_m where province_code = '${pvc_code(i)}' " +
        s"group by category,statis_month")
    ...
    sql(sqlText = s"select * from t_${pvc_code(0)}")
      .join(sql(sqlText = s"select * from t_${pvc_code(1)}"),
        usingColumns = Seq("category", "statis_month"),"full_outer")
    ...
    sql(sqlText = "select category,count(distinct serv_number) c_user,sum(acc_flow_month) s_flow from " +
      s"ads_app_non_2i_${this_month}_m group by category")
    ...
    for( a <- brand_code.indices){
      sql(sqlText = s"select category,count(distinct serv_number) c_user from ads_app_non_2i_${this_month}_m " +
    ...
    sql(sqlText = s"select category,count(distinct serv_number) m0 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month >=0.0 and cur_income_month <= 30.0 group by category")
      .createTempView(viewName = "t_m0_a")

    sql(sqlText = s"select category,count(distinct serv_number) m1 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month > 30 and cur_income_month <= 50 group by category")
      .createTempView(viewName = "t_m1_a")

    sql(sqlText = s"select category,count(distinct serv_number) m2 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month > 50 and cur_income_month <= 100 group by category")
      .createTempView(viewName = "t_m2_a")

    sql(sqlText = s"select category,count(distinct serv_number) m3 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month > 100 and cur_income_month <= 150 group by category")
      .createTempView(viewName = "t_m3_a")

    sql(sqlText = s"select category,count(distinct serv_number) m4 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month > 150 and cur_income_month <= 200 group by category")
      .createTempView(viewName = "t_m4_a")

    sql(sqlText = s"select category,count(distinct serv_number) m5 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month > 200 and cur_income_month <= 250 group by category")
      .createTempView(viewName = "t_m5_a")

    sql(sqlText = s"select category,count(distinct serv_number) m6 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month > 250 and cur_income_month <= 300 group by category")
      .createTempView(viewName = "t_m6_a")

    sql(sqlText = s"select category,count(distinct serv_number) m7 from "+
      s"ads_app_non_2i_${this_month}_m where cur_income_month > 300 group by category")
    ...
    sql(sqlText = "select category,count(distinct serv_number) c_user," +
      s"sum((pack_outer_call_time + pack_inner_call_time)) s_call from ads_app_non_2i_${this_month}_m " +
      "where pack_outer_call_time is not null and pack_inner_call_time is not null group by category")
...
//    user_profile_join.select("cur_product", "prod_id").orderBy($"visit_cnt".desc).limit(10)
    ...
    /**
      ...
      */
        ...
        sql(sqlText = "select cur_product,count(distinct serv_number) as ct0 from ads_app_non_2i_201902_m " +
          "where category = 'bss' group by cur_product").
          join(sql(sqlText = "select cur_product,count(distinct serv_number) as ct0 from ads_app_non_2i_201902_m " +
          "where category = 'cbss' group by cur_product"),usingColumn = "cur_product")
          .write.mode(SaveMode.Overwrite)
          .saveAsTable(tableName = s"ads_app_non_2i_call_${this_month}_m")
