...
object App_Non2i_Again_One_Stop {
  def main(args: Array[String]): Unit = {
    ...
    val user_df_1 = sql(sqlText = "select distinct serv_number,age " +
      s"from ods_user_info_m where statis_month = $this_month  and length(serv_number) = 11")
         //添加性别

    val user_df_2 = sql(sqlText = "select distinct serv_number,age,sex " +
      s"from ods_user_info_m where statis_month = $this_month  and length(serv_number) = 11")
         //添加性别 年龄
//    user_df_1.createTempView(viewName = "user_view")

    val profile_df = sql(sqlText = "select distinct serv_number,province_code,acc_flow_month,brand,cur_income_month,pack_outer_call_time" +
...
//     profile_df.createTempView(viewName = "profile_view")
    ...
    val user_profile_df_1 = profile_df.join(user_df_1, usingColumns =Seq("serv_number"),"left")
    val user_profile_df_2 = profile_df.join(user_df_2, usingColumns =Seq("serv_number"),"left")
      .cache()
//
//    val app_profile_df = app_score_df.join(profile_df,usingColumns =Seq("serv_number"),"full")
    val app_profile_df_2 = profile_df.join(app_score_df,usingColumns =Seq("serv_number"),"inner")
...
//    user_profile_df_1.write.mode(SaveMode.Overwrite).saveAsTable(tableName = s"ads_app_non_2i_1_${this_month}_m")
//    user_profile_df_2.write.mode(SaveMode.Overwrite).saveAsTable(tableName = s"ads_app_non_2i_2_${this_month}_m")
    app_profile_df_2.write.mode(SaveMode.Overwrite).saveAsTable(tableName = s"ads_app_non_2i_4_${this_month}_m")
    val app_profile_df = profile_df.join(app_score_df,usingColumns =Seq("serv_number"),"full")
    app_profile_df.write.mode(SaveMode.Overwrite).saveAsTable(tableName = s"ads_app_non_2i_3_${this_month}_m")
...
//    判断空 或者 age '/t'
    ...
    /**
      ...
      */
    sql(sqlText = s"select category,count(distinct serv_number) m0 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 0 and age <= 14 and sex = 01  group by category")
      .createTempView(viewName = "t_m0")
    sql(sqlText = s"select category,count(distinct serv_number) m1 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 14 and age <= 18 and sex = 01  group by category")
      .createTempView(viewName = "t_m1")
    sql(sqlText = s"select category,count(distinct serv_number) m2 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 18 and age <= 35 and sex = 01  group by category")
      .createTempView(viewName = "t_m2")
    sql(sqlText = s"select category,count(distinct serv_number) m3 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 35 and age <= 55 and sex = 01  group by category")
      .createTempView(viewName = "t_m3")
    sql(sqlText = s"select category,count(distinct serv_number) m4 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 55 and age <= 70 and sex = 01  group by category")
      .createTempView(viewName = "t_m4")
    sql(sqlText = s"select category,count(distinct serv_number) m5 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 70 and age <= 120 and sex = 01  group by category")
      .createTempView(viewName = "t_m5")


    sql(sqlText = s"select category,count(distinct serv_number) f0 from " +
      s"ads_app_non_2i_2_${this_month}_m where age <= 14 and age >0 and sex = 02  group by category")
      .createTempView(viewName = "t_f0")
    sql(sqlText = s"select category,count(distinct serv_number) f1 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 14 and age <= 18 and sex = 02  group by category")
      .createTempView(viewName = "t_f1")
    sql(sqlText = s"select category,count(distinct serv_number) f2 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 18 and age <= 35 and sex = 02  group by category")
      .createTempView(viewName = "t_f2")
    sql(sqlText = s"select category,count(distinct serv_number) f3 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 35 and age <= 55 and sex = 02  group by category")
      .createTempView(viewName = "t_f3")
    sql(sqlText = s"select category,count(distinct serv_number) f4 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 55 and age <= 70 and sex = 02  group by category")
      .createTempView(viewName = "t_f4")
    sql(sqlText = s"select category,count(distinct serv_number) f5 from " +
      s"ads_app_non_2i_2_${this_month}_m where age > 70 and age <= 120 and sex = 02  group by category")
      .createTempView(viewName = "t_f5")
    sql(sqlText = s"select category,count(distinct serv_number) mf6 from " +
      s"ads_app_non_2i_2_${this_month}_m where age < 0 or age > 120 or sex <> 02 or sex <> 01 group by category")
    ...
    for (i <- pvc_code.indices) {
      sql(sqlText = s"select category," +
        ...
        s"ads_app_non_2i_basic_${this_month}_m where province_code = '${pvc_code(i)}' " +
        s"group by category")
...
//      s"ads_app_non_2i_basic_${this_month}_m where acc_flow_month > 0 and acc_flow_month < 102400 group by category ")
...
//      sql(sqlText = s"select category,count(distinct serv_number) c_user from ads_app_non_2i_basic_${this_month}_m " +
...
//
//    sql(sqlText = s"select category,count(distinct serv_number) m0 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month >=0.0 and cur_income_month <= 30.0 group by category")
//      .createTempView(viewName = "t_m0_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m1 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month > 30 and cur_income_month <= 50 group by category")
//      .createTempView(viewName = "t_m1_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m2 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month > 50 and cur_income_month <= 100 group by category")
//      .createTempView(viewName = "t_m2_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m3 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month > 100 and cur_income_month <= 150 group by category")
//      .createTempView(viewName = "t_m3_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m4 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month > 150 and cur_income_month <= 200 group by category")
//      .createTempView(viewName = "t_m4_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m5 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month > 200 and cur_income_month <= 250 group by category")
//      .createTempView(viewName = "t_m5_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m6 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month > 250 and cur_income_month <= 300 group by category")
//      .createTempView(viewName = "t_m6_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m7 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month > 300 and cur_income_month <= 1000 group by category")
//      .createTempView(viewName = "t_m7_a")
//
//    sql(sqlText = s"select category,count(distinct serv_number) m8 from "+
//      s"ads_app_non_2i_basic_${this_month}_m where cur_income_month < 0 or cur_income_month > 1000 group by category")
...
//
//
//    sql(sqlText = "select cur_product,count(distinct serv_number) as ct0 from ads_app_non_2i_basic_201902_m " +
//      "where category = 'bss' group by cur_product").
//      union(sql(sqlText = "select cur_product,count(distinct serv_number) as ct1 from ads_app_non_2i_basic_201902_m " +
...
//      s"sum((pack_outer_call_time + pack_inner_call_time)) s_call from ads_app_non_2i_basic_${this_month}_m " +
