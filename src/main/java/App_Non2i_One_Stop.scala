...
object App_Non2i_One_Stop {
  def main(args: Array[String]): Unit = {
    ...
    val user_df = sql(sqlText = "select distinct serv_number,sex,age,province_code " +
      s"from ods_user_info_m where statis_month = $this_month  and length(serv_number) = 11")
      .cache()
    user_df.createTempView(viewName = "user_view")

    val profile_df = sql(sqlText = "select distinct serv_number,cur_income_month," +
      "pack_outer_call_time,pack_inner_call_time," +
      "brand,is_income,cur_product,acc_flow_month " +
      ...
      "and length(cur_product) = 8 and is_income = '1' and product_type <> '1' and channel_type in ('1030500','1030100','1030300','1030400','2030100','2030300') ")
      .cache()
    // profile_df.createTempView(viewName = "profile_view")

    val user_profile_df = user_df.join(profile_df, usingColumn = "serv_number")
      .cache()
    ...
    user_profile_df.write.mode(SaveMode.Overwrite).saveAsTable(tableName = s"ads_app_non_2i_${this_month}_m")
...
//    }
  ...
  sql(sqlText = "select category,count(distinct serv_number) as ct0 from ads_app_non_2i_201902_m " +
    "where category = 'bss' group by category").
    join(sql(sqlText = "select category,count(distinct serv_number) as ct1 from ads_app_non_2i_201902_m " +
      "where category = 'cbss' group by category"),usingColumn = "category")
    .write.mode(SaveMode.Overwrite)
    .saveAsTable(tableName = s"ads_app_non_2i_call_${this_month}_m")
